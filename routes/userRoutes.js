...
router.get('/hod/dashboard', async (req, res) => {
    try {
        const EMTRF = await EMTRF.find(); // Fetch bookings using the TRF model
        res.render('hodDashboard', { EMTRF }); // Render hodDashboard.ejs with bookingList data
    } catch (err) {
        console.error('Error fetching bookings:', err);
        res.status(500).send('Server error'); // Handle server error
    }
